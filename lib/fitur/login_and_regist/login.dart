...
import 'package:application_english/Home.dart';
  ...
  Widget _buildTitle() {
    return Column(
      children: const [
        ...
        SizedBox(height: 10),
        Text("Please login to your account", style: TextStyle(color: Colors.white70, fontSize: 18)),
      ],
    );
  }
...
Widget _buildLoginForm(BuildContext context, AuthProvider authProvider) {
  return Column(
    children: [
      _buildTextField(usernameController, "Username", Icons.person), // Changed icon to user
      const SizedBox(height: 20),
  ...
  Widget _buildLoginButton(BuildContext context, AuthProvider authProvider) {
    return ElevatedButton(
      onPressed: () {
        ...
        } else if (authProvider.validateCredentials(usernameController.text, passwordController.text)) {
